...
```{r}
# Removing time and character variables for clustering
energy_cluster <- energy_clean[, -c(1,7,8,14,15,17,21)]
```

```{r}
# Hierarchical clustering
energy_dist <- dist(energy_cluster, method = 'euclidian')
hc_energy <- hclust(energy_dist, method = 'complete')
hc_energy
```

```{r}
plot(hc_energy, hang = -0.1)
hc_cluster_id_energy <- cutree(hc_energy, k = 7)
```

# K-means clustering

```{r}
k_energy = kmeans(energy_cluster, 7)
k_energy
k_cluster_id <- k_energy$cluster
```

```{r}
sil_hc_energy <- cluster::silhouette(hc_cluster_id_energy, energy_dist)
sil_k_energy <- cluster::silhouette(k_cluster_id, energy_dist)
```

```{r}
plot(sil_hc_energy, border = 'grey')
plot(sil_k_energy,  border = 'grey')
```

The silhouette plot of the kmeans cluster shows a better result than the silhouette plot of the hierarchical clustering.
...
energy_numeric <- energy_clean[, -c(1,7,8,14,15,17,18,20,21,22,23,24)]
...
```{r}
# Add back preciptype and convert it to a factor
...
```{r}
# writing the formula
energy_formula = energy_mean ~ visibility + windBearing + dewPoint + windSpeed + pressure + humidity +temperatureMax + temperatureLow + temperatureMin + temperatureHigh + moonPhase + precipType 
```
...
```{r}
energy_nn5 <- neuralnet(energy_formula, hidden = 5, data = training_energy)
```
...
```{r}
pred_energy_nn_1 <- compute(energy_nn1, test_energy[,-12])
pred_energy_nn_5 <- compute(energy_nn5, test_energy[,-12])
pred_energy_nn_3_2 <- compute(energy_nn3_2, test_energy[,-12])
```
...
```{r}
pred_energy_n1 <- compute(energy_n1, test_energy[,-12])
pred_energy_n5 <- compute(energy_n5, test_energy[,-12])
pred_energy_n32 <- compute(energy_n32, test_energy[,-12])
```

```{r}
energy_results1 <- data.frame(
  actual = test_energy$energy_mean,
...
```{r}
cor(energy_results1[,'actual'], energy_results1[,c("n5", "n32")])
```
...
```{r}
energy_n11 <- neuralnet(energy_formula, hidden = c(1,1), data = training_energy, stepmax = 1e+06)
```

```{r}
pred_energy_n55 <- compute(energy_n55, test_energy[,-12])
pred_energy_n11 <- compute(energy_n11, test_energy[,-12])
pred_energy_n444 <- compute(energy_444, test_energy[,-12])
```
...
energy_results1 <- data.frame(
  ...
  n444 = pred_energy_n444$net.result
)
```

```{r}
cor(energy_results1[,'actual'], energy_results1[,c("n55", "n11", "n444")])
```
...
```{r}
energy_444 <- neuralnet(energy_formula, hidden = c(4,4,4), data = training_energy, stepmax = 1e+06)
```
